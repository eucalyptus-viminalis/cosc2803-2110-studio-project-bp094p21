   ...
   public int getTotalCasesInDateRange1Country(String country, String from, String to) {
    ...
    public ArrayList<String> get3SimilarCasesCountryMil (String country) {
        ...
        try {
            ...
            Statement statement = connection.createStatement();
            statement.setQueryTimeout(30);
            String query = "select name n, printf('%.0f', (1.0*sum(newcases))/(1.0 * population/1000000)) [permil], printf('%.4f', abs((select (1.0*sum(newcases))/(1.0 * population/1000000) from countryrecords join country on country.id = countryrecords.countryid group by name) - (1.0*sum(newcases))/(1.0 * population/1000000))) difference from countryrecords join country on country.id = countryrecords.countryid group by name order by abs((select (1.0*sum(newcases))/(1.0 * population/1000000) from countryrecords join country on country.id = countryrecords.countryid where name = '" + country + "' group by name) - (1.0*sum(newcases))/(1.0 * population/1000000)) asc limit 4";
            ResultSet results = statement.executeQuery(query);
            while (results.next()) {
                String name     = results.getString("n");
    ...
    public ArrayList<String> get3SimilarCasesUSStateMil (String state) {
        ...
        try {
            ...
            Statement statement = connection.createStatement();
            statement.setQueryTimeout(30);
            String query = "select state.name n, printf('%.0f', (1.0*sum(newcases))/(1.0 * state.population/1000000)) [permil], printf('%.4f', abs((select (1.0*sum(newcases))/(1.0 * state.population/1000000) from staterecords join state on state.id = staterecords.stateid join country on country.id = state.countryid group by state.name) - (1.0*sum(newcases))/(1.0 * state.population/1000000))) difference from staterecords join state on state.id = staterecords.stateid join country on country.id = state.countryid where country.name = 'US' group by state.name having sum(newcases) > 0 order by abs((select (1.0*sum(newcases))/(1.0 * state.population/1000000) from staterecords join state on state.id = staterecords.stateid where name = '" + state + "' group by name) - (1.0*sum(newcases))/(1.0 * state.population/1000000)) asc limit 4";
            ResultSet results = statement.executeQuery(query);
            while (results.next()) {
                String name     = results.getString("n");
                String casespermil = results.getString("permil");
